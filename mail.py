...
def emailsandurls(source_list, m_pattern, u_pattern):
    app_mail_list = []
    app_url_list = []
    for taken in source_list:
        taken = taken.strip('<,>')
                ...
                if not headers["Subject"] and not headers["From"] and not headers["To"] and not Body_raw:
                    ...
                    for item in headers["From"].split():
                        ...
                        if re.findall(mail_pattern, item):
                            From_email.append(item)

                    if not From_email:
                        print "Warning: no e-mail address in From"
                    else:
                    ...
                    if not To_email:
                        print "Warning: no e-mail address in To"
                    else:
                    ...
                    if headers["Bcc"]:
                        for item in headers["Bcc"].split():
                            item = item.strip('<,>')
                            if re.match(mail_pattern, item):
                                Bcc_email.append(item)
                        print "== %s emails in To: ==" % len(Bcc_email)
                        for mail in Bcc_email:
                            print mail

                    Subject_email, Subject_url = emailsandurls(headers["Subject"].split(), mail_pattern, url_pattern)
                    # for item in headers["Subject"].split():
                    #     item = item.strip('<,>')
                    #     if re.match(mail_pattern, parseaddr(item)[1]):
                    #         Subject_email.append(parseaddr(item)[1])
                    #     elif re.match(url_pattern, item):
                    #         Subject_url.append(item)

                    headers_mails = len(From_email) + len(To_email) + len(Subject_email) + len(Cc_email) + len(Bcc_email)

                    Body_email, Body_url = emailsandurls(Body, mail_pattern, url_pattern)
